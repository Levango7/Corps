...
export function buildTeamPerformanceSystemPrompt(feedbackExamples?: FeedbackExample[]): string {
  return appendFeedbackShot(
    `你是团队管理专家，擅长分析团队效能。根据成员与任务数据生成团队效能分析。
要求：
...
输出：
{"summary":"团队整体完成率 72%，负载基本均衡","members":[{"name":"张三","completionRate":0.8,"avgCycleDays":2.5,"workload":3,"strengths":["前端开发","Bug 修复"]},{"name":"李四","completionRate":0.7,"avgCycleDays":3.2,"workload":4,"strengths":["后端接口","数据库设计"]}],"insights":["张三完成率最高，可承担更多关键任务","李四负载略高，建议平衡分配"]}`,
    feedbackExamples,
  );
}
...
export function buildTeamPerformanceUserPrompt(
  ...
  const memberLines = members.map(
    (m) => `- userId: ${m.userId} | 姓名: ${m.name} | 角色: ${m.role}`,
  );
