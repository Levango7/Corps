...
export async function PATCH(
  ...
  const fail = (status: number, key: ApiMsgKey, reason?: string) =>
    NextResponse.json(
  ...
  try {
    ...
    const result = await runWithWorkspace(
      ...
      async (tx) => {
        ...
        if (
          input.action === "recall" &&
          now.getTime() - message.createdAt.getTime() > recallWindowMs
        ) {
        ...
        const changed = await tx.message.updateMany({
          where: {
            ...
            revokedAt: null,
            ...(input.action === "recall"
              ? { createdAt: { gte: new Date(now.getTime() - recallWindowMs) } }
              : {}),
          },
          data:
            input.action === "recall"
        ...
        return {
          status: "ok" as const,
          message: updated.isRecalled ? { ...updated, attachments: [] } : updated,
        };
      },
      userId,
    );
