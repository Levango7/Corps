...
export async function POST(
  ...
  try {
    ...
    const result = await runWithWorkspace(wid, async (tx) => {
      ...
      const targetMember = await tx.member.findFirst({ where: { userId: validated.delegateToId, workspaceId: wid } });
      if (!targetMember) return { kind: "targetNotFound" as const };

      const updatedNodes = [...nodes];
      updatedNodes[instance.currentNode] = { ...currentNode, approverUserId: validated.delegateToId, approverRole: undefined };

      const operation = await tx.approvalOperation.create({
        data: { instanceId: aid, workspaceId: wid, operatorId: ctx.payload.sub, action: "delegate", nodeIndex: instance.currentNode, comment: validated.comment, delegateToId: validated.delegateToId, nodeId: `node_${instance.currentNode}` },
      });
      ...
      await tx.notification.create({ data: { userId: validated.delegateToId, workspaceId: wid, type: "approval_delegate", entityId: aid, entityTitle: instance.title } });
    ...
    if (result.kind === "targetNotFound") return NextResponse.json({ code: 403, message: apiMsg(req, "approvalDelegateTargetNotFound"), data: null }, { status: 403 });
