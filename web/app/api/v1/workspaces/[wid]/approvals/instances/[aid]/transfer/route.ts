...
export async function POST(
  ...
  try {
    ...
    const result = await runWithWorkspace(
      ...
      async (tx) => {
        ...
        const targetMember = await tx.member.findFirst({
          where: { userId: validated.transferToId, workspaceId: wid },
        });
        ...
        updatedNodes[instance.currentNode] = {
          ...currentNode,
          approverUserId: validated.transferToId,
    ...
    if (result.kind === "targetNotFound")
      return NextResponse.json(
        { code: 403, message: apiMsg(req, "approvalTransferTargetNotFound"), data: null },
        { status: 403 },
      );
