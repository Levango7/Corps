...
export async function POST(
  ...
  try {
    ...
    const result = await runWithWorkspace(wid, async (tx) => {
      ...
      const targetMember = await tx.member.findFirst({ where: { userId: validated.transferToId, workspaceId: wid } });
      if (!targetMember) return { kind: "targetNotFound" as const };

      const updatedNodes = [...nodes];
      updatedNodes[instance.currentNode] = { ...currentNode, approverUserId: validated.transferToId, approverRole: undefined };

      const operation = await tx.approvalOperation.create({
        data: { instanceId: aid, workspaceId: wid, operatorId: ctx.payload.sub, action: "transfer", nodeIndex: instance.currentNode, comment: validated.comment, transferToId: validated.transferToId, nodeId: `node_${instance.currentNode}` },
      });
      ...
      await tx.notification.create({ data: { userId: validated.transferToId, workspaceId: wid, type: "approval_transfer", entityId: aid, entityTitle: instance.title } });
    ...
    if (result.kind === "targetNotFound") return NextResponse.json({ code: 403, message: apiMsg(req, "approvalTransferTargetNotFound"), data: null }, { status: 403 });
