...
export async function POST(
  ...
  try {
    ...
    const result = await runWithWorkspace(
      ...
      async (tx) => {
        ...
        const targetMember = await tx.member.findFirst({
          where: { userId: validated.addSignToId, workspaceId: wid },
        });
        ...
        if (nodeMode === "sequential") {
          ...
          updatedNodes[instance.currentNode] = {
            ...currentNode,
            additionalApprovers: [
              ...(currentNode.additionalApprovers ?? []),
              validated.addSignToId,
            ],
          };
        }
        ...
        const operation = await tx.approvalOperation.create({
          data: {
            ...
            addSignToId: validated.addSignToId,
        ...
        await tx.notification.create({
          data: {
            userId: validated.addSignToId,
            workspaceId: wid,
            type: "approval_add_sign",
            entityId: aid,
    ...
    if (result.kind === "targetNotFound")
      return NextResponse.json(
        { code: 403, message: apiMsg(req, "approvalAddSignTargetNotFound"), data: null },
        { status: 403 },
      );
