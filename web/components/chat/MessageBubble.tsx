...
function highlightText(text: string, query: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    testRegex.test(part) ? (
      <mark
        key={i}
        className="bg-[var(--accent-soft)] text-[var(--accent)] rounded-[var(--radius-sm)] px-0.5"
      >
        {part}
      </mark>
    ) : (
...
export function MessageBubble({
  ...
  const updateMessage = async (action: "recall" | "edit") => {
    ...
    try {
      ...
      const result: { code?: number; data?: EditableChatMessage; reason?: string } =
        await response.json();
      if (!response.ok || result.code !== 0 || !result.data || result.data.id !== message.id) {
        throw new Error(
          result.reason === "recallExpired"
            ? t("recallExpired")
    ...
    >
      ...
      <div className={`min-w-0 max-w-[70%] ${isOwn ? "items-end" : "items-start"} flex flex-col`}>
        ...
        {canUpdate && !editing && (
          <details
            ref={menuRef}
            className="relative mb-0.5 opacity-100 sm:opacity-0 group-hover/message:opacity-100 group-focus-within/message:opacity-100 open:opacity-100"
            onBlur={(event) => {
          ...
          >
            <summary
              ...
              title={t("moreActions")}
              className="flex cursor-pointer list-none items-center justify-center rounded-[var(--radius-sm)] p-1 text-[var(--meta)] hover:bg-[var(--surface-2)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-ring)]"
            >
            ...
            <div className="absolute right-0 top-full z-20 min-w-[140px] rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-1 shadow-[var(--elev-md)] text-[length:var(--text-sm)] text-[var(--fg)]">
              <button
                ...
                className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] p-2 hover:bg-[var(--surface-2)] disabled:opacity-50"
              >
                <Pencil size={14} />
                {t("edit")}
              </button>
              <button
                type="button"
                disabled={saving || !canRecall}
                title={!canRecall ? t("recallExpired") : undefined}
                onClick={() => void updateMessage("recall")}
                className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] p-2 text-[var(--danger-fg)] hover:bg-[var(--danger-soft)] disabled:opacity-50"
              >
                <Undo2 size={14} />
                {t("revoke")}
              </button>
        ...
        >
          {isRecalled ? (
            <span>{t("messageRevoked")}</span>
          ) : editing ? (
            <form
              onSubmit={(event) => {
                event.preventDefault();
                void updateMessage("edit");
              }}
            ...
            >
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className="rounded-[var(--radius-sm)] px-2 py-1 text-[var(--fg-2)] hover:bg-[var(--surface-3)] disabled:opacity-50"
                >
                  {t("cancel")}
                </button>
                <button
                  ...
                  className="flex items-center gap-1 rounded-[var(--radius-sm)] bg-[var(--accent)] px-2 py-1 text-[var(--accent-fg)] disabled:opacity-50"
                >
        ...
        {error && (
          <p role="alert" className="mt-1 text-[length:var(--text-xs)] text-[var(--danger-fg)]">
            {error}
          </p>
        )}
        {!isRecalled && message.editedAt && (
          <span className="mt-0.5 text-[length:var(--text-xs)] text-[var(--meta)]">
            {t("edited")}
          </span>
        )}
        {/* 已读回执（仅自己的消息显示） */}
