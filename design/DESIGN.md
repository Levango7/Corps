...
|------|--------------------------|---------------------------|------|
| 页面底 | `#F8F8F6` / 暗 `#0E0F12` | `--bg` | 全局背景（微暖灰） |
| 卡片底 | `#FEFEFC` / 暗 `#16181D` | `--surface` | 卡片/弹窗/输入底 |
| 次级底 | `#F4F4F1` / 暗 `#1E2026` | `--surface-2` | 悬停/内嵌区 |
| 三级底 | `#ECECE8` / 暗 `#262932` | `--surface-3` | 条纹/分隔区 |
| 主文字 | `#1A1A18` / 暗 `#F2F3F5` | `--fg` | 标题/正文 |
| 次文字 | `#3D3D3A` / 暗 `#C7CBD3` | `--fg-2` | 小标题/强调正文 |
| 辅助文字 | `#6B6B68` / 暗 `#8B919E` | `--muted` | 副信息（非长正文） |
| 元数据 | `#9A9A96` / 暗 `#6A707C` | `--meta` | 时间戳/计数 |
| 边框 | `#E5E5E0` / 暗 `#272A31` | `--border` | 默认 1px 边框 |
| 发丝边框 | `#F0F0EC` / 暗 `#1E2026` | `--border-soft` | 行内分隔 |
...
- **`--shell-blue` 解耦决策**：外壳三层背景不直接引用 `--accent`（钴蓝 #4263EB），而是单独定义 `--shell-blue: #6BA3D8`（柔和天蓝，降饱和、偏湖蓝）。原因：`--accent` 是品牌强调色（按钮/选中态），若外壳底布也用 `--accent` 派生，浅色下大面积淡钴蓝会显紫味，与"中性克制"定位冲突。`--shell-blue` 与 `--accent` 解耦后，外壳保持纯蓝调宁静质感，品牌色仅出现在 ≤2 处强调点；租户换肤覆盖 `--accent` 时外壳底色不受影响。此决策于 v0.4.0 落地（design-tokens.css :60-62）。
...
`--radius-sm: 8px`（按钮/输入）· `--radius-md: 10px`（Tab/小卡）· `--radius-lg: 14px`（卡片）· `--radius-xl: 16px`（弹窗/大卡）· `--radius-pill: 9999px`（头像/胶囊标签）。
...
| 日程（Home 右栏，v2 占位） | `Calendar` | 20 | 工作区 Home 右栏标题（SPEC §3 日历 v2 时机考虑，MVP 仅设计占位） |
...
- **主区（非对称 2:1）**：
  ...
  - **右 1/3 — 今日 / 最近 / 日程**：
    ...
    - 日程（`Calendar`，v2 占位）：团队周会 16:00–17:00 · 会议室 A（SPEC §3 日历 v2 时机考虑，MVP 仅设计占位）
- **5 态**：
...
- **日程 `/w/:wid/calendar`（v2 占位设计，非 MVP 范围 — SPEC §3 日历明确不做）**：2026 年 8 月真实月历（周一始），今日 22 高亮；会议事件点（accent/success/warn 三色）；连接平台后自动同步（设计占位，MVP 不实现）。
