...
> 日期：2026-08-26 ｜ 状态：**ACCEPTED（2026-08-26 用户拍板通过）** ｜ 决策人：用户（产品负责人）
...
> **v2 更新（2026-09-02 用户拍板）**：Pro 定价调整为 ¥29.9/人/月、¥299/人/年（替代原 ¥59/¥590）；Free 清单扩至 14 项（全部已建成）、Pro 清单扩至 8 项（新增无限席位首位卖点、附件 10MB→50MB、每周任务摘要邮件）。清单文本以翻译键存于 messages（pricing.features.*），本表保留逻辑结构供阅读；决策依据见 `docs/market/pricing-redesign-v2.md`。
...
export const PRICING_PLANS = {
  free: {
    ...
    cta: "免费开始",
    features: [
      // features.free.f01–f14（pricing.features.free.* 翻译键，zh/en 双语）
      "任务看板（看板/列表双视图 + 拖拽改状态）",
      "任务详情（负责人 / 截止日 / 优先级 / 状态 / 标签 / 里程碑）",
      "成员邀请 + Owner/Admin/Member 三级角色",
      "任务评论 + @提及提醒",
      "任务内轻沟通 IM（含附件上传、实时推送）",
      "决策记录（每工作区最近 10 条，版本留痕）",
      "通知中心（指派 / @提及 / 决策更新提醒）",
      "日历连接（Google / Outlook，截止日同步）",
      "Cmd+K 全局搜索（任务 + 决策记录）",
      "批量操作（多选移动 / 指派 / 删除）",
      "中英双语界面",
      "深色模式",
      "移动端响应式（手机浏览器全功能）",
      "降级不锁数据（回退免费版后数据只读可导出）",
    ],
  },
  pro: {
    name: "专业版 Pro",
    ...
    badge: "推荐",
    features: [
      // features.pro.p00–p07（pricing.features.pro.* 翻译键）
      "免费版全部能力",
      "无限席位（11 人及以上团队）",
      "无限决策记录 + 版本留痕 + 任务双向回链",
      "任务筛选与自定义视图",
      "附件存储扩容（单文件 10MB → 50MB）",
      "CSV 数据导出（任务 + 决策记录）",
      "邮件通知（指派 / 截止日 / @提及 + 每周任务摘要）",
      "优先邮件支持（1 个工作日内响应）",
    ],
  },
} as const;
...
> 口径来源：Free 功能集 = v0.2.1 已建成能力全量（见 pricing-redesign-v2.md §1.1 代码证据表）；Pro 差异项与 `web/app/[locale]/w/[wid]/billing/page.tsx` PLANS 同步。「决策记录最近 10 条」为免费层软上限。「附件 10MB→50MB」已在 attachments 路由按 active 订阅门控实现。「每周任务摘要」由 /api/cron/weekly-digest（仅 active 订阅工作区）每周一执行。

### 4.2 功能对比表

表：Free 与 Pro 功能对比表（v2，七分组；group/feature 以 pricing.matrix.* 翻译键存储）

| 功能 | Free | Pro |
|------|------|-----|
| **任务协作** |||
| 任务看板 / 列表双视图 + 拖拽改状态 | ✅ | ✅ |
| 任务详情字段（负责人/截止日/优先级/状态/标签/里程碑） | ✅ | ✅ |
| 评论 + @提及 | ✅ | ✅ |
| 任务内轻沟通 IM（附件 + 实时推送） | ✅ | ✅ |
| 批量操作（多选移动 / 指派 / 删除） | ✅ | ✅ |
| 任务筛选与自定义视图 | — | ✅ |
...
| 附件单文件大小 | 10MB | 50MB |
| 成员变更自动同步席位数量 | — | ✅ |
| 价格 | ¥0 | ¥29.9/人/月 或 ¥299/人/年 |

> 「✅/—」使用文本字符而非 emoji（SPEC §10 禁 emoji 约束针对 UI 图标体系；表格内符号同样以纯文本呈现，避免引入非 Lucide 视觉元素）。基础邮件通知归 Free（邀请依赖邮件通道），每周摘要为 Pro 独占——与 pricing-redesign-v2.md §2.3 口径一致。
...
> ✅ 第 3、6 条涉及的对外承诺已于 2026-08-26 经用户拍板批准生效：发票时点承诺（国内主体就绪后开放增值税发票能力）与年付 14 天退款政策均获确认，按上文文案执行。
...
依赖与风险：定价数字与 FAQ 第 3/6 条承诺已于 2026-08-26 经用户拍板确认（pricing-strategy.md §8 清单）；`KanbanSquare` 等 lucide-react 导出名以锁定版本的实测为准（SPEC §11 已知坑）。